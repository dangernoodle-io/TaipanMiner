...
// handshake (TA-232), the drought/keepalive timers, keepalive-id tracking,
// and the stratum-does-not-block-the-caller isolation invariant
// (TA-233/TA-494).
#include "unity.h"
...
typedef struct {
    ...
    int              reset_calls;
    bool             have_result;
    stratum_result_t queued_result;
} fake_work_t;

static fake_work_t s_fw;

static bool fake_publish(void *ctx, const stratum_job_t *job)
{
    (void)ctx; (void)job;
    s_fw.publish_calls++;
    return true;
...
static bool fake_drain(void *ctx, stratum_result_t *out)
{
    (void)ctx;
    if (s_fw.have_result) {
...
// structural guarantee instead: with the transport PERMANENTLY failing
// every connect, repeated stratum_fsm_service() calls each return control
// to the caller promptly enough that a co-running counter, bumped once per
// iteration BETWEEN calls, reaches its full count -- i.e. stratum_fsm never
// enters an internal unbounded retry loop that would starve the caller's
// task of a chance to run anything else (WiFi, mgmt-plane, ...).
// ---------------------------------------------------------------------------
