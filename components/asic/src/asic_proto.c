...
size_t asic_build_cmd(uint8_t *buf, size_t buflen, uint8_t cmd, uint8_t group,
    ...
    // buf[0] = 0x55 (ASIC_PREAMBLE_TX_0)
    // buf[1] = 0xAA (ASIC_PREAMBLE_TX_1)
    buf[0] = ASIC_PREAMBLE_TX_0;
    buf[1] = ASIC_PREAMBLE_TX_1;

    // buf[2] = header byte: ASIC_TYPE_CMD | group | cmd
...
size_t asic_build_job(uint8_t *buf, size_t buflen, const asic_job_t *job) {
    ...
    // buf[2] = header byte: ASIC_TYPE_JOB | ASIC_GROUP_SINGLE | ASIC_CMD_WRITE
    buf[2] = ASIC_TYPE_JOB | ASIC_GROUP_SINGLE | ASIC_CMD_WRITE;

    // buf[3] = length byte: ASIC_JOB_DATA_LEN + 4 = 82 + 4 = 86
    // (length = job_data(82) + header(1) + length(1) + crc16(2))
    buf[3] = ASIC_JOB_DATA_LEN + 4;

    // buf[4..85] = memcpy of the 82-byte job struct
    memcpy(buf + 4, (const uint8_t *)job, ASIC_JOB_DATA_LEN);

    // Compute CRC16 over header(1) + length(1) + job(82) = 84 bytes
    uint16_t crc = crc16_false(buf + 2, ASIC_JOB_DATA_LEN + 2);
...
void asic_difficulty_to_mask(double difficulty, uint8_t mask_out[4])
{
    ...
    // ASIC ticket mask = (largest_power_of_2(diff) - 1), BE, per-byte bit-reversed
