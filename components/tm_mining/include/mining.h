...
// ---------------------------------------------------------------------------
// Producer surface (TA-561): delivery-agnostic POD snapshot + pure
// gather() + static bb_serialize descriptor (format-neutral). No I/O, no
// allocation, no transport, and no serialization-format knowledge live
// here. Registration of mining_desc + mining_gather happens in the
// composition root (src/main.c PRODUCER REGISTRATION SLOT), not here —
// adding a new delivery path (UDP push, etc.) later is one binding added
// at the root; this file is never reopened for it.
// ---------------------------------------------------------------------------
