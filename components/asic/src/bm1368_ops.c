...
{
    pll_params_t pll;
    pll_calc(freq_mhz, BM1368_FB_MIN, BM1368_FB_MAX, &pll);
    uint8_t vdo = pll_vdo_scale(&pll);
    uint8_t postdiv = pll_postdiv_byte(&pll);
    write_reg(BM1368_REG_PLL, vdo, (uint8_t)pll.fb_div, pll.refdiv, postdiv);
...
// --- BM1368 chip init sequence ---
static esp_err_t bm1368_chip_init(void)
{
    uint16_t addr_interval = 256 / BM1368_CHIP_COUNT;

    // Step 1: Set version mask x3
    for (int i = 0; i < 3; i++) {
        write_reg(BM1368_REG_VERSION, 0x90, 0x00, 0xFF, 0xFF);
        vTaskDelay(pdMS_TO_TICKS(10));
    }

    // Step 2: Read chip ID (broadcast)
    uint8_t read_data[2] = {0x00, BM1368_REG_CHIP_ID};
    send_cmd(ASIC_CMD_READ, ASIC_GROUP_ALL, read_data, 2);

    // Wait and read chip responses
    int chip_count = 0;
    for (int i = 0; i < BM1368_CHIP_COUNT + 1; i++) {
    ...
    if (chip_count == 0) {
        bb_log_e(TAG, "no BM1368 chips detected");
    ...
    write_reg(BM1368_REG_A8, 0x00, 0x07, 0x00, 0x00);
    vTaskDelay(pdMS_TO_TICKS(5));

    // Step 5: MISC_CTRL broadcast — different from BM1370
    write_reg(BM1368_REG_MISC_CTRL, 0xFF, 0x0F, 0xC1, 0x00);
    vTaskDelay(pdMS_TO_TICKS(5));
    ...
    write_reg(BM1368_REG_CORE_CTRL, 0x80, 0x00, 0x8B, 0x00);
    vTaskDelay(pdMS_TO_TICKS(5));
    // BM1368-specific: 0x80 0x00 0x80 0x18 instead of 0x0C
    write_reg(BM1368_REG_CORE_CTRL, 0x80, 0x00, 0x80, 0x18);
    vTaskDelay(pdMS_TO_TICKS(5));

    // Step 9: Ticket mask placeholder — re-written after baud switch + freq ramp (step 16)
    vTaskDelay(pdMS_TO_TICKS(5));

    // Step 10: IO driver strength — different from BM1370
    write_reg(BM1368_REG_IO_DRV, 0x02, 0x11, 0x11, 0x11);
    vTaskDelay(pdMS_TO_TICKS(5));

    // Step 11: Per-chip register writes
    for (int i = 0; i < chip_count; i++) {
        uint8_t addr = (uint8_t)(i * addr_interval);
        write_reg_chip(addr, BM1368_REG_A8, 0x00, 0x07, 0x01, 0xF0);
        write_reg_chip(addr, BM1368_REG_MISC_CTRL, 0xF0, 0x00, 0xC1, 0x00);
        write_reg_chip(addr, BM1368_REG_CORE_CTRL, 0x80, 0x00, 0x8B, 0x00);
        write_reg_chip(addr, BM1368_REG_CORE_CTRL, 0x80, 0x00, 0x80, 0x18);
        write_reg_chip(addr, BM1368_REG_CORE_CTRL, 0x80, 0x00, 0x82, 0xAA);
        vTaskDelay(pdMS_TO_TICKS(5));
    }

    // Step 12: Misc registers
    write_reg(BM1368_REG_MISC_SET, 0x00, 0x00, 0x44, 0x80);
    vTaskDelay(pdMS_TO_TICKS(5));
    // BM1368-specific: 0x03 instead of 0x02
    write_reg(BM1368_REG_ANALOG_MUX, 0x00, 0x00, 0x00, 0x03);
    vTaskDelay(pdMS_TO_TICKS(5));
    write_reg(BM1368_REG_MISC_SET, 0x00, 0x00, 0x44, 0x80);
    vTaskDelay(pdMS_TO_TICKS(5));
    write_reg(BM1368_REG_CORE_CTRL, 0x80, 0x00, 0x8D, 0xEE);
    vTaskDelay(pdMS_TO_TICKS(5));

    // Step 13: Baud rate switch to 1 Mbps
    write_reg(BM1368_REG_FAST_UART, 0x11, 0x30, 0x02, 0x00);
    ...
    float target_freq = (float)BM1368_DEFAULT_FREQ_MHZ;
    ...
    write_reg(BM1368_REG_HASH_COUNT, 0x00, 0x00, 0x1E, 0xB5);
    vTaskDelay(pdMS_TO_TICKS(5));
    ...
    bb_log_i(TAG, "BM1368 init complete");
    return ESP_OK;
}

static bb_err_t bm1368_chip_quiesce(void)
{
...
static bb_err_t bm1368_chip_resume(void)
{
