// Host test harness entry point — TA-561 mining engine.
//
// NOTE (reported to stratum lane): as of this PR, the native env's
// build_src_filter/lib wiring does not yet include components/mining's
// sources, so this file will not link until that wiring lands (see PR
// report). Tests are written and correct; execution is blocked on the
// native-env scaffolding stratum is settling.
#include <unity.h>

// test_mining_self_test_flag_default_false must run before
// test_mining_set_self_test_failed_flips_flag (process-static state).
