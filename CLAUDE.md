...
- `make build` — build all boards (tdongle-s3, esp32-wroom32, esp32-s2-mini, esp32-c3-supermini, bitaxe-403, bitaxe-601, bitaxe-650)
...
| `esp32-s2-mini` | ESP32-S2 Mini (LOLIN) | none (SW mining) | USB CDC |
| `esp32-c3-supermini` | ESP32-C3 SuperMini | none (SW mining) | USB Serial/JTAG |
...
Both `esp32-s2-mini` and `esp32-c3-supermini` are single-core; the miner task runs on core 0 alongside WiFi/Stratum. The C3 SuperMini 0.42" OLED (SSD1306) is a deferred follow-up pending pinout confirmation.
...
- **Namespace isolation**: BB-owned keys (`wifi_ssid`, `wifi_pass`, `hostname`, `mdns_en`, `update_check_en`, `display_en`) live in BB's default `"bb_cfg"` namespace. TM-owned keys stay in `"taipanminer"`.
- **Components consumed**:
...
- Mining hot-loop data must be cache-isolated: any variable read by `mine_nonce_range` on a per-nonce basis (currently `block2[64]`, `hw_backend_ctx_t hw_ctx`) on the PIN=y (S3/AHB) path is declared `static __attribute__((aligned(64)))` — **64 bytes** is the S3 D-cache line size (32 bytes was correct only for classic-ESP32 uncached DRAM — do not regress to 32). Avoid embedding large structs in `mining_stats_t` (it sits in the hot-loop BSS region): keep it lean so cumulative global growth doesn't perturb adjacent hot-data placement — `pool_stats` was extracted to its own TU for this reason (TA-413), with a `static_assert` capping the struct size. NOTE: the magnitude of layout-driven throughput loss is modest and board-dependent — the TA-392/PR#435 case was real, but the TA-413/#413 "3.6%" was not reproduced by a targeted revert (wroom32 measured ~95% of its kernel ceiling on v0.52), so treat this as a code-hygiene guideline, not a confirmed perf lever. The `mine_nonce_range` Tier-2 log emits an `eff: NN.N%` column (live kH/s vs the boot SHA microbench ceiling); sustained drops vs steady state are the tripwire. Measured steady state: esp32-wroom32 ≈ 95% (DPORT); use the per-board boot microbench line as each board's reference rather than a fixed number.
