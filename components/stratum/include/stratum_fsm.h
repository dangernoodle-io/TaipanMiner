...
// Table-driven stratum protocol client, replacing the pre-rebuild
// goto-reconnect loop (stratum_io.c) with bb_fsm over bb_tcp_client (via the
// stratum_transport_ops_t seam -- see stratum_transport.h).
//
...
// task running elsewhere (TA-233/TA-494). See test_stratum_isolation_*.
...
typedef struct {
    ...
    // producer snapshot (stratum_snap.c).
