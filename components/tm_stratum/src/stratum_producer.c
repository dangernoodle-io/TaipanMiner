...
bb_err_t stratum_gather(void *vctx, void *vout)
{
    ...
    // pool_host/pool_port are sourced from ctx->cfg, which is NOT the live
    // connection target on ESP: stratum_transport_esp_init() loads the
    // actual endpoint from NVS (bb_tcp_client_init("tm_stratum", NULL, ...))
    // and esp_connect() ignores the host/port args entirely (see
    // stratum_transport_esp.c). bb_tcp_client's public API currently exposes
    // no resolved-endpoint getter (only connected/last_ok_ms/fail_count/
    // tls_error_code via bb_tcp_client_health_fill()) -- until one exists
    // (TA-562), this snapshot can drift from the live pool if cfg.host/port
    // and the NVS-provisioned endpoint ever disagree.
    ...
    out->fsm_state       = (int64_t)stratum_fsm_state(ctx);
